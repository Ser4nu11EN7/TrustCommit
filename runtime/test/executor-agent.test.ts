...
class FakeProvider implements ModelProvider {
  ...
  public async generateArtifact(_task: TaskRecord, repoContext: Record<string, unknown>, _context: ProviderContext) {
...
test("executor agent exports artifact and grounded agent log", async () => {
  ...
  const agentLog = JSON.parse(fs.readFileSync(result.logPath, "utf8")) as {
    ...
    verification: { schemaSatisfied: boolean; missingFields: string[] };
    ...
    receiptChain: {
      ...
      onchain: { proofHash: string; submitTxHash: string | null };
    };
